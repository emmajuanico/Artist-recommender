...
def compute_mean_audio_features(tracks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    artist_means = mitjana_artistes.reset_index()
    
    artist_ids = artist_means['artist_id']
...
def create_similarity_graph(artist_audio_features_df: pd.DataFrame, similarity: str, out_filename: str = None) -> nx.Graph:
    ...
    for i in range(num_artists):
        for j in range(i + 1, num_artists):
            ...
            art1=artist_audio_features_df.iloc[i].name
            art2=artist_audio_features_df.iloc[j].name
            if art1 not in similarity_graph.nodes():
                similarity_graph.add_node(art1)
            if art2 not in similarity_graph.nodes():
                similarity_graph.add_node(art2)
            similarity_graph.add_edge(art1, art2, weight=similarity_matrix[i][j])
